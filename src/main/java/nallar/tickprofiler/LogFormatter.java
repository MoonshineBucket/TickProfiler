...
public class LogFormatter extends Formatter {
	...
	public String format(LogRecord record) {
		...
		formattedMessage.append('[').append(loggerName == null ? "" : loggerName).append("] ")
		...
		if(throwable != null) {
			if(throwable.getStackTrace().length == 0) formattedMessage.append("Stack trace unavailable." +
					" Add -XX:-OmitStackTraceInFastThrow to your java parameters to see all stack traces.")
				...
				formattedMessage.append(throwable.getClass() == Throwable.class ? stackTraceWriter.toString()
		...
		if((writtenSize += formattedMessage.length()) > (1024 * 1024 * 50)) { // 50MB
			...
			Log.disableDiskWriting("No more log messages will be recorded to disk," +
					" exceeded 50MB log size.");
		}
