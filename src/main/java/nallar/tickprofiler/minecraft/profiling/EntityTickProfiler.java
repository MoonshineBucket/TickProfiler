...
public class EntityTickProfiler {
	...
	private Method isActiveChunk = getIsActiveChunkMethod();
	...
	public static EntityTickProfiler ENTITY_TICK_PROFILER = new EntityTickProfiler();
	public static ProfileCommand.ProfilingState profilingState = ProfileCommand.ProfilingState.NONE;
	private HashMap<Class<?>, AtomicInteger> invocationCount = new HashMap<Class<?>, AtomicInteger>();
	private HashMap<Class<?>, AtomicLong> time = new HashMap<Class<?>, AtomicLong>();
	private HashMap<Object, AtomicLong> singleTime = new HashMap<Object, AtomicLong>();
	private HashMap<Object, AtomicLong> singleInvocationCount = new HashMap<Object, AtomicLong>();
	private int ticks;
	...
	public static synchronized boolean startProfiling(ProfileCommand.ProfilingState profilingState_) {
		if(profilingState != ProfileCommand.ProfilingState.NONE) return false;
	...
	public boolean startProfiling(final Runnable runnable, ProfileCommand.ProfilingState state,
								  final int time, final Collection<World> worlds_) {
		...
		synchronized(EntityTickProfiler.class) {
			if(!startProfiling(state)) return false;
		...
		for(int i = 0; i < toTick.size(); i++) {
			...
			if(!entity.isDead) {
				try {
					...
					CrashReportCategory category = crashReport.makeCategory("Entity being ticked");
			...
			if(entity.isDead) {
				...
				if(toTick.size() <= i || toTick.get(i) != entity) toTick.remove(entity);
				else toTick.remove(i--);
	...
	public void runTileEntities(World world, ArrayList<TileEntity> toTick) {
		...
		while(iterator.hasNext()) {
			...
			if(!isActiveChunk(world, x >> 4, z >> 4)) continue;
			if(!tileEntity.isInvalid() && tileEntity.hasWorldObj() && chunkProvider.chunkExists(x >> 4, z >> 4)) {
				try {
					tileEntity.updateEntity();
				} catch (Throwable throwable) {
					CrashReport crashReport = CrashReport.makeCrashReport(throwable, "Ticking tile entity");
					CrashReportCategory category = crashReport.makeCategory("Tile entity being ticked");
					tileEntity.func_85027_a(category);
			...
			if(tileEntity.isInvalid()) {
				...
				if(chunkProvider.chunkExists(tileEntity.xCoord >> 4, tileEntity.zCoord >> 4)) {
					...
					if(chunk != null) chunk.cleanChunkBlockTileEntity(tileEntity.xCoord & 15,
	...
	public synchronized void tick() {
		if(profilingState != ProfileCommand.ProfilingState.NONE) ticks++;
	}
	...
	public void writeJSONData(File file) throws IOException {
		...
		tables.add(0, CollectionsUtil.map("TPS", tps));
	...
	public TableFormatter writeData(TableFormatter tf, int elements) {
		...
		List<Object> sortedSingleKeysByTime = sortedKeys(singleTime, elements);
		for(Object o : sortedSingleKeysByTime) {
			tf.row(niceName(o)).row(singleTime.get(o) / (1000000d * singleInvocationCount.get(o).get()))
					.row((singleTime.get(o) / totalTime) * 100);
		}

		tf.finishTable();
		tf.sb.append('\n');
		Map<ChunkCoords, ComparableLongHolder> chunkTimeMap = new HashMap<ChunkCoords, ComparableLongHolder>() {
			@Override
		...
		for(final Map.Entry<Object, Long> singleTimeEntry : singleTime.entrySet()) {
			...
			} else throw new RuntimeException("Wrong type: " + o.getClass());
			if(x != Integer.MIN_VALUE) chunkTimeMap.get(new ChunkCoords(x, z,
					dimension)).value += singleTimeEntry.getValue();
		}
		...
		for(ChunkCoords chunkCoords : sortedKeys(chunkTimeMap, elements)) {
			...
			tf.row(chunkCoords.dimension + ": " + chunkCoords.chunkXPos + ", " + chunkCoords.chunkZPos)
					.row(chunkTime / (1000000d * ticks)).row((chunkTime / totalTime) * 100);
		}
		...
		for(Class c : sortedKeys(time, elements)) {
			tf.row(niceName(c)).row(time.get(c) / (1000000d * ticks))
		...
		for(Class c : sortedKeys(timePerTick, elements)) {
			tf.row(niceName(c)).row(timePerTick.get(c) / 1000000d)
	...
	private static Object niceName(Object o) {
		if(o instanceof TileEntity) return niceName(o.getClass()) + ' ' + ((TileEntity) o).xCoord +
				',' + ((TileEntity) o).yCoord + ',' + ((TileEntity) o).zCoord + ':' + getDimension((TileEntity) o);
		else if(o instanceof Entity) return niceName(o.getClass()) + ' ' + (int) ((Entity) o).posX + ',' +
				(int) ((Entity) o).posY + ',' + (int) ((Entity) o).posZ + ':' + getDimension((Entity) o);
	...
	private static String niceName(Class<?> clazz) {
		...
		if(name.contains(".")) {
			...
			if(pName.contains(".")) pName = pName.substring(pName.lastIndexOf('.') + 1);
	...
	private boolean isActiveChunk(World world, int chunkX, int chunkZ) {
		if(isActiveChunk == null) return true;
		if(lastCX == chunkX && lastCZ == chunkZ) return cachedActive;
	...
	private static final class ChunkCoords {
		...
		public boolean equals(Object o) {
			return o instanceof ChunkCoords && ((ChunkCoords) o).chunkXPos == this.chunkXPos &&
					((ChunkCoords) o).chunkZPos == this.chunkZPos && ((ChunkCoords) o).dimension == this.dimension;
		}
