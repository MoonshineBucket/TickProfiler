...
public class DumpCommand extends Command {
	...
	public void processCommand(final ICommandSender commandSender, List<String> arguments) {
		...
		try {
			if(commandSender instanceof Entity) world = ((Entity) commandSender).worldObj;

			x = Integer.parseInt(arguments.remove(0));
			y = Integer.parseInt(arguments.remove(0));
			z = Integer.parseInt(arguments.remove(0));

			if(!arguments.isEmpty()) world = DimensionManager.getWorld(Integer
					.parseInt(arguments.remove(0)));
		} catch (Exception e) {
			world = null;
		}

		if(world == null) sendChat(commandSender, "Usage: /dump x y z [world=currentworld]");
		sendChat(commandSender, dump(new TableFormatter(commandSender), world, x, y, z,
				commandSender instanceof Entity ? 35 : 70).toString());
	}
	...
	public static TableFormatter dump(TableFormatter tf, World world, int x, int y, int z, int maxLen) {
		...
		if(blockId < 1) sb.append("No block at ").append(Log.name(world)).append(" x,y,z")
