...
public class ProfileCommand extends Command {
	...
	public void processCommand(final ICommandSender commandSender, List<String> arguments) {
		...
		try {
			...
			if(arguments.size() > 2) world = DimensionManager.getWorld(Integer.valueOf(arguments.get(2)));
			else if (location && commandSender instanceof Entity) world = ((Entity) commandSender).worldObj;
			if(location && x == null) {
		...
		if(!entityTickProfiler.startProfiling(new Runnable() {
			@Override
			public void run() {
				sendChat(commandSender, entityTickProfiler
						.writeStringData(new TableFormatter(commandSender)).toString());
			}
		...
		if(location) entityTickProfiler.setLocation(x, z);
		sendChat(commandSender, "Profiling for " + time + " seconds in " + (world == null ?
				"all worlds " : Log.name(world)) + (location ? " at " + x + ',' + z : ""));
	}
