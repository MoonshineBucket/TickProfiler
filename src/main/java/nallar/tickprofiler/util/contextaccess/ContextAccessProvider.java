...
class ContextAccessProvider {
	...
	private static Class[] contextAccessClasses = {ContextAccessReflection.class, ContextAccessSecurityManager.class,};
		...
		for(Class<?> clazz : contextAccessClasses) {
			try {
				...
				if(currentClass != ContextAccessProvider.class) {
					...
					throw new Error("Wrong class returned: " + currentClass +
							", expected ContextAccessProvider. " + sb);
				}

				return contextAccess;
			} catch (Throwable t) {
				if(Log.debug) Log.debug("Unable to set up context access class " + clazz +
						". " + t.getMessage() + ", falling back to slower context access. On JRE: " +
